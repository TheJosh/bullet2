...
// New behavior of joint motor at limit
// See discussion http://bulletphysics.org/Bullet/phpBB3/viewtopic.php?f=9&t=4746
//#define D6_APPLY_MOTOR_AT_LIMIT
...
btScalar getMotorFactorAway(btScalar pos, btScalar lowLim, btScalar uppLim, btScalar maxVel, btScalar timeFact)
{
	if(lowLim > uppLim)
	{
	...
	btScalar delta_max = maxVel / timeFact;
	if(pos < lowLim) 
	{
		if(pos >= (lowLim - delta_max))
		{
			lim_fact = (pos - lowLim + delta_max) / delta_max;
		}
		else
		{
	...
	{
		if(pos <= (uppLim + delta_max))
		{
			lim_fact = (uppLim - pos + delta_max) / delta_max;
		}
		else
		{
			lim_fact = btScalar(0.0f);
	...
	btConstraintInfo2 *info, int row, btVector3& ax1, int rotational,int rotAllowed)
{
    ...
    {   // if the joint is powered, or has joint limits, add in the extra row
        ...
        info->m_lowerLimit[srow] = btScalar(0.f);
        info->m_upperLimit[srow] = btScalar(0.f);
		btScalar eps = 0.001f;
#if !defined(D6_APPLY_MOTOR_AT_LIMIT)
        if (powered)
        {

//			btVector3 vel = getRigidBodyB().getLinearVelocity();
//			limot->m_currentLimitError -= vel[0] / info->fps;
...
//                info->m_lowerLimit[srow] += -limot->m_maxMotorForce;
//                info->m_upperLimit[srow] += limot->m_maxMotorForce;
//				info->m_constraintError[srow] += limot->m_targetVelocity * 0.01f;
//			}
///*
			else
			{
				btScalar mot_fact = getMotorFactorAway(	limot->m_currentPosition, 
													limot->m_loLimit,
													limot->m_hiLimit, 
													limot->m_maxMotorForce * 0.1f, 
													info->fps * info->fps * limot->m_stopERP);
//				info->m_constraintError[srow] += mot_fact * limot->m_targetVelocity;
//                info->m_lowerLimit[srow] = -limot->m_maxMotorForce * mot_fact  - 1.E20f * (1.f - mot_fact);
//                info->m_upperLimit[srow] = limot->m_maxMotorForce * mot_fact + 1.E20f * (1.f - mot_fact);
			}
//*/
        }
#endif
...
//					if(!powered)
					info->m_upperLimit[srow] = SIMD_INFINITY;
//					info->m_upperLimit[srow] += limot->m_currentLimitError * 1.E14f;
                }
                else
                {
//					if(!powered)
                    info->m_lowerLimit[srow] = -SIMD_INFINITY;
//                    info->m_lowerLimit[srow] += limot->m_currentLimitError * 1.E14f;
...
//                        if (body1)
		...
		if(powered)
		{
			printf("%f : %f : %e\n",limot->m_currentLimitError, info->m_constraintError[srow],  info->m_upperLimit[srow]);
		}
#ifdef D6_APPLY_MOTOR_AT_LIMIT
		if (powered)
		{
			btScalar tag_vel = rotational ? limot->m_targetVelocity : -limot->m_targetVelocity;
			btScalar mot_fact = getMotorFactor( limot->m_currentPosition,
												...
												tag_vel,
												info->fps * info->erp);
			btScalar motorError = mot_fact * limot->m_targetVelocity;
			if (limit)
			{
				btScalar fudgeFact = 10.f;
				info->cfm[srow] = limot->m_stopCFM;
				// only apply motor if we are moving away from limit 
				// and motor force is greater than current constraint force
				if((info->m_constraintError[srow] < btScalar(0.f)) && (motorError < info->m_constraintError[srow]))
				{
					info->m_constraintError[srow] = motorError;
//					info->m_lowerLimit[srow] = -limot->m_maxMotorForce  * fudgeFact;
//					info->m_upperLimit[srow] = 0.f;
				}
				else if ((info->m_constraintError[srow] > btScalar(0.f)) && (motorError > info->m_constraintError[srow]))
				{
					info->m_constraintError[srow] = motorError;
//					info->m_lowerLimit[srow] = 0.f;
//					info->m_upperLimit[srow] = limot->m_maxMotorForce * fudgeFact;
				}
			}
			else
			{
