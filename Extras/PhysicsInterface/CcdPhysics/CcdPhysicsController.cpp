...
btSimdVector3 startVel(0,0,0);//-10000);
...
btSimdTransform	CcdPhysicsController::GetTransformFromMotionState(PHY_IMotionState* motionState)
{
	btSimdTransform trans;
	float tmp[3];
	motionState->getWorldPosition(tmp[0],tmp[1],tmp[2]);
	trans.setOrigin(btSimdVector3(tmp[0],tmp[1],tmp[2]));
...
void CcdPhysicsController::CreateRigidbody()
{
	...
	btSimdTransform trans = GetTransformFromMotionState(m_cci.m_MotionState);
...
bool		CcdPhysicsController::SynchronizeMotionStates(float time)
{
	...
	if (!m_body->IsStatic())
	{
		const btSimdVector3& worldPos = m_body->getCenterOfMassPosition();
		m_MotionState->setWorldPosition(worldPos[0],worldPos[1],worldPos[2]);
		
		const btSimdQuaternion& worldquat = m_body->getOrientation();
		...
		btSimdVector3 scaling(scale[0],scale[1],scale[2]);
		GetCollisionShape()->setLocalScaling(scaling);
	} else
	{
		btSimdVector3 worldPos;
		btSimdQuaternion worldquat;

		m_MotionState->getWorldPosition(worldPos[0],worldPos[1],worldPos[2]);
		m_MotionState->getWorldOrientation(worldquat[0],worldquat[1],worldquat[2],worldquat[3]);
		btSimdTransform oldTrans = m_body->getCenterOfMassTransform();
		btSimdTransform newTrans(worldquat,worldPos);
...
void		CcdPhysicsController::RelativeTranslate(float dlocX,float dlocY,float dlocZ,bool local)
{
	if (m_body)
	{
		...
		btSimdVector3 dloc(dlocX,dlocY,dlocZ);
		btSimdTransform xform = m_body->getCenterOfMassTransform();
...
void		CcdPhysicsController::RelativeRotate(const float rotval[9],bool local)
{
	if (m_body )
	{
		...
		btSimdMatrix3x3 drotmat(	rotval[0],rotval[1],rotval[2],
								rotval[4],rotval[5],rotval[6],
								rotval[8],rotval[9],rotval[10]);


		btSimdMatrix3x3 currentOrn;
...
void CcdPhysicsController::GetWorldOrientation(btSimdMatrix3x3& mat)
{
	float orn[4];
	m_MotionState->getWorldOrientation(orn[0],orn[1],orn[2],orn[3]);
	btSimdQuaternion quat(orn[0],orn[1],orn[2],orn[3]);
	mat.setRotation(quat);
}

void		CcdPhysicsController::getOrientation(float &quatImag0,float &quatImag1,float &quatImag2,float &quatReal)
{
	btSimdQuaternion q = m_body->getCenterOfMassTransform().getRotation();
	quatImag0 = q[0];
	quatImag1 = q[1];
	quatImag2 = q[2];
	quatReal = q[3];
...
void		CcdPhysicsController::setOrientation(float quatImag0,float quatImag1,float quatImag2,float quatReal)
{
	...
	btSimdTransform xform  = m_body->getCenterOfMassTransform();
	xform.setRotation(btSimdQuaternion(quatImag0,quatImag1,quatImag2,quatReal));
	m_body->setCenterOfMassTransform(xform);
...
void		CcdPhysicsController::setPosition(float posX,float posY,float posZ)
{
	...
	btSimdTransform xform  = m_body->getCenterOfMassTransform();
	xform.setOrigin(btSimdVector3(posX,posY,posZ));
	m_body->setCenterOfMassTransform(xform);
...
{
	const btSimdTransform& xform = m_body->getCenterOfMassTransform();
...
void		CcdPhysicsController::setScaling(float scaleX,float scaleY,float scaleZ)
{
	...
	{
		m_cci.m_scaling = btSimdVector3(scaleX,scaleY,scaleZ);
...
{
	btSimdVector3 torque(torqueX,torqueY,torqueZ);
	btSimdTransform xform = m_body->getCenterOfMassTransform();
	if (local)
	{
...
{
	btSimdVector3 force(forceX,forceY,forceZ);
	btSimdTransform xform = m_body->getCenterOfMassTransform();
	if (local)
	{
...
{
	btSimdVector3 angvel(ang_velX,ang_velY,ang_velZ);
...
void		CcdPhysicsController::SetLinearVelocity(float lin_velX,float lin_velY,float lin_velZ,bool local)
{
	...
	btSimdVector3 linVel(lin_velX,lin_velY,lin_velZ);
...
{
	btSimdVector3 impulse(impulseX,impulseY,impulseZ);

	if (impulse.length2() > (SIMD_EPSILON*SIMD_EPSILON))
	{
		m_body->activate();
		
		btSimdVector3 pos(attachX,attachY,attachZ);
...
{
	const btSimdVector3& linvel = this->m_body->getLinearVelocity();
	linvX = linvel.x();
	linvY = linvel.y();
	linvZ = linvel.z();
...
{
	const btSimdVector3& angvel= m_body->getAngularVelocity();
	angVelX = angvel.x();
	angVelY = angvel.y();
	angVelZ = angvel.z();
}
...
{
	btSimdVector3 pos(posX,posY,posZ);
	btSimdVector3 rel_pos = pos-m_body->getCenterOfMassPosition();
	btSimdVector3 linvel = m_body->getVelocityInLocalPoint(rel_pos);
	linvX = linvel.x();
	linvY = linvel.y();
	linvZ = linvel.z();
}
...
void		CcdPhysicsController::setRigidBody(bool rigid)
{
	...
	{
		//fake it for now
		btSimdVector3 inertia = m_body->getInvInertiaDiagLocal();
		inertia[1] = 0.f;
