...
void btGpu3DGridBroadphaseOCL::calcHashAABB()
{
	...
	{	// need to pre-fill sime device buffers once
		...
		btGpuDemo3dOCLWrap::copyArrayToDevice(btGpuDemo3dOCLWrap::m_dBodiesHash, m_hBodiesHash, m_maxHandles * 2 * sizeof(unsigned int));
		// now fill the rest (bitonic sorting works with size == pow of 2)
		int remainder = btGpuDemo3dOCLWrap::m_hashSize - m_maxHandles;
		if(remainder)
		{
			btGpuDemo3dOCLWrap::copyArrayToDevice(	btGpuDemo3dOCLWrap::m_dBodiesHash, 
													m_hBodiesHash, 
		...
		btGpuDemo3dOCLWrap::copyArrayToDevice(btGpuDemo3dOCLWrap::m_dPairBuff, m_hPairBuff, m_maxHandles * m_maxPairsPerBody * sizeof(unsigned int));
		firstCall = false;
	}
...
//	btGpuDemo3dOCLWrap::copyArrayToDevice(btGpuDemo3dOCLWrap::m_dBodiesHash, m_hBodiesHash, sizeof(unsigned int) * 2 * m_numHandles);
	...
	btGpuDemo3dOCLWrap::runKernelWithWorkgroupSize(GPUDEMO3D_KERNEL_CLEAR_CELL_START, m_numCells);
...
void btGpu3DGridBroadphaseOCL::findPairsLarge()
{
	...
	{
		btGpuDemo3dOCLWrap::setKernelArg(GPUDEMO3D_KERNEL_FIND_PAIRS_LARGE, 6, sizeof(int),(void*)&m_numLargeHandles);
