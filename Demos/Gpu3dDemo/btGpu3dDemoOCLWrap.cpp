...
void btGpuDemo3dOCLWrap::initKernels()
{
	...
	if(m_broadphaseInited)
	{
		...
		setKernelArg(GPUDEMO3D_KERNEL_FIND_PAIRS_LARGE, 6, sizeof(cl_mem),(void*)&m_dBpParams);
...
void btGpuDemo3dOCLWrap::copyArrayToDevice(cl_mem device, const void* host, unsigned int size)
{
    cl_int ciErrNum;
    ciErrNum = clEnqueueWriteBuffer(m_cqCommandQue, device, CL_TRUE, 0, size, host, 0, NULL, NULL);
    oclCHECKERROR(ciErrNum, CL_SUCCESS);
}

void btGpuDemo3dOCLWrap::copyArrayFromDevice(void* host, const cl_mem device, unsigned int size)
{
    cl_int ciErrNum;
    ciErrNum = clEnqueueReadBuffer(m_cqCommandQue, device, CL_TRUE, 0, size, host, 0, NULL, NULL);
    oclCHECKERROR(ciErrNum, CL_SUCCESS);
}
