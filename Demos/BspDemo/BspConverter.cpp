...
{
	{
		btSimdVector3 playerStart (0.f, 0.f, 100.f);
		...
		for (int i=0;i<bspLoader.m_numleafs;i++)
		{
			...
			{
				std::vector<btSimdVector3> planeEquations;
			...
			for (i=0;i<bspLoader.m_num_entities;i++)
				{
					...
					if ( !strcmp( cl, "trigger_push" ) ) {
						btSimdVector3 targetLocation(0.f,0.f,0.f);
...
void	BspConverter::getVerticesFromPlaneEquations(const std::vector<btSimdVector3>& planeEquations , std::vector<btSimdVector3>& verticesOut )
{
	const int numbrushes = planeEquations.size();
	// brute force:
	for (int i=0;i<numbrushes;i++)
	{
		const btSimdVector3& N1 = planeEquations[i];
		

		for (int j=i+1;j<numbrushes;j++)
		{
			const btSimdVector3& N2 = planeEquations[j];
				
			for (int k=j+1;k<numbrushes;k++)
			{

				const btSimdVector3& N3 = planeEquations[k];

				btSimdVector3 n2n3; n2n3 = N2.cross(N3);
				btSimdVector3 n3n1; n3n1 = N3.cross(N1);
				btSimdVector3 n1n2; n1n2 = N1.cross(N2);
					 ...
					 ( n1n2.length2() > 0.0001f ) )
				{
					...
					if (SimdFabs(quotient) > 0.000001f)
					{
						...
						n3n1 *= N2[3];
						n1n2 *= N3[3];
						btSimdVector3 potentialVertex = n2n3;
...
bool	BspConverter::isInside(const std::vector<btSimdVector3>& planeEquations, const btSimdVector3& point, float	margin)
{
