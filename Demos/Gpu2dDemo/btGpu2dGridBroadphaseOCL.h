...
#ifndef BTGPU2DGRIDBROADPHASEOCL_H
#define BTGPU2DGRIDBROADPHASEOCL_H
...
struct bt2dGridBroadphaseParamsOCL
{
	float m_worldMin[4];
	float m_cellSize[4];
	int   m_gridSize[4];
...
class btGpu2dGridBroadphaseOCL : public btGpu3DGridBroadphase
{
protected:
    // GPU data
	// moved to btGpuDemo3dOCLWrap for now
public:
	btGpu2dGridBroadphaseOCL(	btOverlappingPairCache* overlappingPairCache,
	...
	virtual ~btGpu2dGridBroadphaseOCL();
protected:
	void _initialize();
	void _finalize();
...
#endif //BTGPU2DGRIDBROADPHASEOCL_H