...
{
	// TODO: Replace with btAssert to be simpler
	inline void 
	btOpenCLCheckErr(cl_int err, const char * name)
	{
		if (err != CL_SUCCESS) {
	...
	public:
		...
		bool InitOpenCLDevice()
		{
			...
			if (err != CL_SUCCESS) btAssert( "Context::Context()" );

			m_devices = m_context.getInfo<CL_CONTEXT_DEVICES>();
			if( m_devices.size() <= 0 ) btAssert( "devices.size() > 0" );
		
			m_queue = cl::CommandQueue(m_context, m_devices[0], 0, &err);
		    if (err != CL_SUCCESS) btAssert( "CommandQueue::CommandQueue()");
		}
	};
